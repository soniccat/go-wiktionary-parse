...
func TestPageWorkerV2_1(t *testing.T) {
	...
	inserts := processWikitext("w", wikitext, nil, nil)
...
func TestPageWorkerV2_14(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage14)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage14_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_15(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage15)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage15_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_16(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage16)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage16_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_17(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage17)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage17_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_18(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage18)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage18_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_19(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage19)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage19_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}

func TestPageWorkerV2_20(t *testing.T) {
	wikitext, err := parseWikitext(TestWikiPage20)
	assert.Nil(t, err)

	inserts := processWikitext("w", wikitext, nil, nil)

	print(InsertsToString(inserts))
	assert.Equal(t, TestWikiPage20_parsed, InsertsToString(inserts))
	assert.True(t, len(inserts) > 0)
}
